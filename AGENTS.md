...
当前已接受基线为：76 项测试通过，`ruff check src tests` 通过。

注意：`.github/workflows/ci.yml` 当前执行 `ruff check .`，但部分历史一次性实验脚本
仍存在格式问题。不得把全仓库描述为 Ruff-clean，也不得假设最新分支已经通过远端
CI。如果调整 CI 范围，应明确记录这一决定；如果保留全仓库 lint，则应在独立、
可审查的修改中清理脚本，并按影响范围重新运行实验或 smoke test。
...
5. 开发中先运行相关测试，交付前运行完整 76 项测试和 `ruff check src tests`。
